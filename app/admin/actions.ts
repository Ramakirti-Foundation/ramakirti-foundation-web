...
export async function createInitiativeAction(formData: FormData) {
  ...
  const galleryFiles = formData.getAll('newGallery') as File[];
...
export async function updateInitiativeAction(id: string, formData: FormData) {
  ...
  const existingInit = await db.initiative.findUnique({ where: { id } });
  if (!existingInit) throw new Error('Not found');

  let image_url = existingInit.image_url;
  ...
  const gallery_descriptions = JSON.parse(formData.get('galleryDescriptions') as string || '[]');
  ...
  for (const file of newGalleryFiles) {
    if (file && file.size > 0) {
      const gBuffer = Buffer.from(await file.arrayBuffer());
      newGalleryUrls.push(`data:${file.type};base64,${gBuffer.toString('base64')}`);
    }
  }

  const gallery_urls = [...existingGalleryUrls, ...newGalleryUrls];
