...
const INITIATIVES = [
  { id: 'education', label: '📚 Education', desc: '₹1000 funds a child\'s notebooks and stationery for 1 month' },
  { id: 'food', label: '🍚 Food Program', desc: '₹300 provides 10 hot meals' },
  { id: 'women', label: '💪 Women Empowerment', desc: '₹1000 supports 1 woman sanitary and hygiene kits for 1 month ' },
...
const IMPACT_MAP: Record<string, (amount: number) => string> = {
  education: (a) => `Your ₹${a.toLocaleString('en-IN')} each will fund ${Math.floor(a / 250)} children's monthly stationery & books`,
  food: (a) => `Your ₹${a.toLocaleString('en-IN')} each will provide ${Math.floor(a / 30)} nutritious hot meals`,
  women: (a) => `Your ₹${a.toLocaleString('en-IN')} each will support ${Math.floor(a / 1000)} women in vocational training`,
  general: (a) => `Your ₹${a.toLocaleString('en-IN')} each will go towards the most urgent need at the Foundation`,
};
